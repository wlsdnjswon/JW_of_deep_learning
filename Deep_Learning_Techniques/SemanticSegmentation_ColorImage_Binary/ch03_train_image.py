...
#데이터셋 위치
image_directory = './patch_image/'
mask_directory = './patch_mask/'
...
#외부 이미지를 가져와 예측 
test_img_other = cv2.imread('./patch_image/[0001]BtmBF1_22.png',  cv2.COLOR_BGR2RGB)
test_img_other_input = np.expand_dims(test_img_other,0)
test_img_other_input = image_dataset = np.array(image_dataset) /255.
#test_img_other_input = normalize(np.array(test_img_other_input), axis=(2,3))
prediction_other = (model.predict(test_img_other_input)[0,:,:,:] > 0.1).astype(np.uint8)
...
plt.subplot(232)
plt.title('Testing Label')
plt.imshow(prediction_other[:,:,0], cmap='gray')
plt.show()
